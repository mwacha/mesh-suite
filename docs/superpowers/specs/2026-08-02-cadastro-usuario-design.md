...
1. **Correção pós-investigação**: o rename de `Usuario`→`User`/`Papel`→`Role` não toca só Pedido — `Usuario`/`Papel`/`UsuarioRepository` são usados por todo o núcleo de autenticação (`AuthService`, `AuthContextService`, `AuthController`, `JwtService`, `JwtAuthenticationFilter`, `PasswordResetService`, `TenantQueryService`) e por fixtures de login em `ParceiroControllerTest`/`ProdutoControllerTest`, além do Pedido — **26 arquivos** ao todo (main + test). É mudança mecânica (import + nome de tipo/campo, sem alteração de comportamento), mas toca a área que o próprio PRD-09 marca como exigindo cautela extra (autenticação/sessão). A suíte completa do backend precisa passar antes de mergear, com atenção especial aos testes de login/logout/redefinição de senha/RLS.
