...
`handleParceiroNaoEncontrado→handlePartnerNotFound` (captura `PartnerNotFoundException`) · `handleDocumentoDuplicado` (nome do método já genérico o suficiente, mantém — captura `DuplicateDocumentException`, checar nome exato no arquivo) · `handleParceiroValidacao→handlePartnerValidation` (captura `PartnerValidationException`). Mensagens de erro (`e.getMessage()`, texto literal em português) inalteradas.
...
2. **Blast radius maior que o Empresa, comparável ao Venda→Sale**: módulo próprio com 20 arquivos (controller/service/repository/specifications/exceptions/enums/DTOs/entidade filha) + migration, mais ~19 arquivos de teste/consumidores em 4 módulos backend, mais 8 arquivos de frontend (1 renomeado + 7 com bridge-patch de literais). O `Parceiro` também é a entidade mais "rica" em campos vista até agora nesta iniciativa (endereço completo + inscrições fiscais).
3. **Atenção ao Task 6 da última vez**: o sub-projeto Empresa encontrou um bug real no script de seed dev (`R__seed_dev_tenant.sql`) que não foi pego pela auditoria original (só `.java` foi varrido). Para este sub-projeto, verificar também arquivos `.sql` de seed/dados de exemplo que possam referenciar `parceiro`/`razao_social`/`documento` da tabela antiga.
