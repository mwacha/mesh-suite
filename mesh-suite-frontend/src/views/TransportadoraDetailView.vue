<!-- mesh-suite-frontend/src/views/TransportadoraDetailView.vue -->
<template>
  <AppShell title="Transportadora">
    <p v-if="erro" class="error-geral">{{ erro }}</p>

    <div class="detalhe">
      <aside class="rail">
        <input v-model="buscaRail" class="busca-rail" placeholder="Buscar transportadora..." @input="carregarRail" />
        <div
...
const tabs = ['Dados', 'Endereços', 'Contatos'] as const
...
async function carregarParceiro(id: string) {
  ...
  try {
    ...
    erro.value = 'Não foi possível carregar os dados da transportadora.'
  }
}
...
async function carregarRail() {
  erro.value = ''
  try {
    const pagina = await listPartners({ busca: buscaRail.value || undefined, papel: 'CARRIER', page: 0, size: 10 })
...
function editar() {
  router.push({ name: 'transportadoras-editar', params: { id: parceiroId.value } })
}
